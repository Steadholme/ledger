...
//! request. When audit is disabled (the dev/test default) the sink is a no-op — no channel, no
//! worker.
//!
//! Tempo emits notable scheduler outcomes: `tempo.job.fail` (a cron fire failed) and
//! `tempo.heartbeat.miss` (a dead-man heartbeat went silent past grace). Producers fill only the
//! logical fields (actor / action / target / severity / detail).
...
impl AuditEvent {
    ...
    /// `info`-severity event (normal lifecycle: a recovered heartbeat, a job edit).
    ...
    /// `warning`-severity event (a failed cron fire or a missed heartbeat).
...
impl AuditSink {
    ...
    /// `watchtower_url` is unparseable — those misconfigurations only warn and turn audit OFF; they
    /// never fail startup or affect the request path. Must be called from within a tokio runtime
    /// when enabled (the worker is `tokio::spawn`ed).
...
/// Drain the queue and POST each event. Delivery failures only warn — an event is never retried and
/// a down Watchtower never affects anything but this background task.
...
/// Parsed Watchtower target: where to connect (`host`/`port`), the `Host` header authority, and the
/// request `path` (`<base>/events`).
struct Target {
    host: String,
    port: u16,
    authority: String,
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::warning("tempo.job.fail", "scheduler", "job", "boom"));
        }
    ...
    fn event_serializes_to_shared_fields() {
        let ev = AuditEvent::warning("tempo.heartbeat.miss", "scheduler", "nightly-backup", "no beat for 600s");
        ...
        assert_eq!(v["action"], "tempo.heartbeat.miss");
        assert_eq!(v["severity"], "warning");
        assert_eq!(v["source"], "tempo");
    }
