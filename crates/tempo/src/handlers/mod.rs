...
//! page, matching the HOLDFAST enterprise brand. All producer-supplied text (job names, URLs) is
...
/// Embedded design system (Odyssey canonical CSS + Tempo service CSS), inlined into
...
pub fn render_error(
    ...
    let body = ERROR_HTML
        .replace("{{CSS}}", app_css())
        .replace("{{SHIELD}}", SHIELD_SVG)
        .replace("{{USERBOX}}", &userbox("Tempo", email))
