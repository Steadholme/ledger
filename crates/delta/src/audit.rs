...
//! `WATCHTOWER_URL/events` (`Authorization: Bearer AUDIT_INGEST_TOKEN`) over plain HTTP/1.1 with a
//! short timeout. [`AuditSink::emit`] is sync + infallible: it `try_send`s and DROPS (warn +
...
//!
//! Delta only emits the logical fields (actor / action / target / severity / detail). The opaque
//! event payload NEVER rides an audit record — appends are SAMPLED and carry only the stream name
//! and the allocated seq in `detail`.
...
impl AuditEvent {
    ...
    /// `info`-severity event (normal lifecycle: sampled append).
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::info("delta.stream.append", "svc", "orders", "seq=1"));
        }
    ...
    /// An append event serializes to exactly the safe shared fields — never any payload bytes.
    #[test]
    fn append_serializes_safe_fields_only() {
        let ev = AuditEvent::info("delta.stream.append", "service", "orders", "seq=42");
        ...
        assert_eq!(v["action"], "delta.stream.append");
        assert_eq!(v["source"], "delta");
    }
